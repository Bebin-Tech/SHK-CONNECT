    ...
    <div className="p-4 sm:p-6 relative min-h-screen flex flex-col items-center justify-center bg-[radial-gradient(circle_at_top_right,_#FDFCFB_0%,_#E2D1C3_100%),radial-gradient(circle_at_bottom_left,_#E3F2FD,_#BBDEFB)]" style={{
      ...
      <div className="mb-5 flex items-center justify-center gap-3 sm:absolute sm:top-6 sm:left-8 sm:mb-0 sm:gap-6 z-20">
        <div className="w-16 h-16 sm:w-24 sm:h-24 transform hover:scale-110 transition-all duration-500">
          <img src="/api/static/img/shk_logo_new.png" alt="Logo" className="w-full h-full object-contain filter drop-shadow-md" />
        </div>
      ...
      <div className="bg-white/95 backdrop-blur-md border border-white/20 p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] w-full relative z-10" style={{ maxWidth: '450px' }}>
        <div className="text-center mb-6 sm:mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold text-[#1A237E]">Sign In</h2>
          <p className="text-gray-400 text-sm mt-2 font-medium">Welcome back to SHK Industries</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          ...
          <div className="flex items-center justify-between gap-3">
            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
              <input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} className="w-5 h-5 sm:w-4 sm:h-4 rounded border-gray-300 text-[#1A237E] focus:ring-[#1A237E]" />
              <span className="whitespace-nowrap">Remember me</span>
            </label>
            <Link to="/signup" className="text-sm text-[#1A237E] font-semibold hover:underline whitespace-nowrap">Create Account</Link>
          </div>
        ...
        <p className="text-center mt-8 text-[#1A237E] text-sm font-bold">version 2.0</p>
      </div>
    </div>
  );
}
